...
SSHRemote.cmd = function(url, cmd) {
	var plink = this.getPlinkCmd(url, cmd);
	if (plink) {
		Shell.system(plink);
		return true;
	};
...
SSHRemote.sudoCmd = function(url, cmd) {
	var plink = this.getPlink(url);
	if (plink) {
		var password = this.getPassword(url);
		if (password) {
...
SSHRemote.forwardCmd = function(url, nextUrl, cmd) {
	var nextPlink = this.getPlinkCmd(nextUrl, cmd);
	if (nextPlink) {
		var plink = this.getPlinkCmd(url, nextPlink);
		if (plink) {
			Shell.system(plink);
			return true;
		};
...
SSHRemote.forwardSudoCmd = function(url, nextUrl, cmd) {
	var nextPlink = this.sudoGetCmdEncoded(nextUrl, cmd);
	if (nextPlink) {
		var plink = this.getPlinkCmd(url, nextPlink);
		if (plink) {
			Shell.system(plink);
